      ...
      module Concerns
        ...
        private
          ...
          def where_query_method(resource)
            <<-STR
              def where(filter: Api::Data::MarketFilter.new, params: {})
                Api::ORM::Requester.load resource: :#{resource}, filter: filter, params: params
              end
            STR
          end
          ...
          def find_query_method(find_attr)
            <<-STR
              def find(id)
                where filter: Api::Data::MarketFilter.new(#{find_attr}: [id])
              end
            STR
          end
